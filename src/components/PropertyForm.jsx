...
export default function PropertyForm({ mode, initialForm, onCancel, onSave, onDirtyChange }) {
  ...
  // One automatic lookup per property, ever. The address key used to include
  // `zip || city`, so filling in the city and THEN the zip produced two
  // different keys and fired the lookup twice — 6 Rentcast calls to add one
  // property instead of 3. Manual "Re-fetch" is unaffected.
  const autoDoneRef = useRef(false);
  const streetRef = useRef(null);

  // Leaving a half-filled form is the easiest way to lose real work here, so the
  // page above us needs to know whether there is anything worth confirming. It
  // guards in-app navigation; beforeunload covers refresh / closing the tab.
  ...
  const doLookup = async (addr, { force } = {}) => {
    ...
    // A forced re-fetch is a deliberate spend; let it through but don't let the
    // automatic path fire again afterwards.
  ...
  const maybeAutoLookup = (addr) => {
    ...
    if (num(f.value) > 0) return; // already have a value — nothing to fetch
    ...
    <div className="prop-form">
      ...
      <div className="address-grid">
        <label className="field">
          ...
          <input
            ref={streetRef}
            className={streetError ? 'has-error' : ''}
            value={f.street}
          ...
          />
          {streetError && <span className="field-error">{streetError}</span>}
        </label>
        <label className="field">
          City
          <input
